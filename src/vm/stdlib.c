...
static const char *color_code_for_symbol(const char *symbol) {
  if (strcmp(symbol, ".red") == 0)
    return "\x1b[31m";
  if (strcmp(symbol, ".green") == 0)
    return "\x1b[32m";
  if (strcmp(symbol, ".yellow") == 0)
    return "\x1b[33m";
  if (strcmp(symbol, ".blue") == 0)
    return "\x1b[34m";
  if (strcmp(symbol, ".magenta") == 0)
    return "\x1b[35m";
  if (strcmp(symbol, ".cyan") == 0)
    return "\x1b[36m";
...
static Value vm_native_add(VM *vm, int arg_count, Value *args) {
  ...
  return (Value){VALUE_NUMBER,
                 {.number = AS_NUMBER(args[0]) + AS_NUMBER(args[1])}};
}
...
static Value vm_native_math_atan2(VM *vm, int arg_count, Value *args) {
  ...
  if (arg_count < 2 || !IS_NUMBER(args[0]) || !IS_NUMBER(args[1]))
    return (Value){VALUE_NULL, {0}};
  return (Value){VALUE_NUMBER,
                 {.number = atan2(AS_NUMBER(args[0]), AS_NUMBER(args[1]))}};
}
...
static Value vm_native_math_pow(VM *vm, int arg_count, Value *args) {
  ...
  if (arg_count < 2 || !IS_NUMBER(args[0]) || !IS_NUMBER(args[1]))
    return (Value){VALUE_NULL, {0}};
  return (Value){VALUE_NUMBER,
                 {.number = pow(AS_NUMBER(args[0]), AS_NUMBER(args[1]))}};
}
...
static Value vm_native_substr(VM *vm, int arg_count, Value *args) {
  if (arg_count < 3 || !IS_STRING(args[0]) || !IS_NUMBER(args[1]) ||
      !IS_NUMBER(args[2])) {
    return (Value){VALUE_NULL, {0}};
  }
...
static int format_value_to_buffer(Value value, char *buf, size_t max_len) {
  if (IS_NUMBER(value)) {
    ...
    for (size_t i = 0; i < array->count; i++) {
      if (written >= max_len)
        break;
      res = format_value_to_buffer(array->elements[i], buf + written,
                                   max_len - written);
      if (res < 0)
        return -1;
      written += (size_t)res;
...
static Value vm_native_file_write(VM *vm, int arg_count, Value *args) {
  (void)vm;
  if (arg_count < 3 || !IS_STRING(args[1]) || !IS_STRING(args[2]))
    return (Value){VALUE_BOOL, {.boolean = 0}};
  const char *path = AS_CSTRING(args[1]);
...
static Value vm_native_array_remove(VM *vm, int arg_count, Value *args) {
  (void)vm;
  if (arg_count < 2 || !IS_ARRAY(args[0]) || !IS_NUMBER(args[1]))
    return (Value){VALUE_NULL, {0}};
  ObjArray *array = (ObjArray *)AS_OBJ(args[0]);
  int idx = (int)AS_NUMBER(args[1]);
  if (idx < 0 || idx >= (int)array->count)
    return (Value){VALUE_NULL, {0}};
...
static Value vm_native_array_insert(VM *vm, int arg_count, Value *args) {
  (void)vm;
  if (arg_count < 3 || !IS_ARRAY(args[0]) || !IS_NUMBER(args[1]))
    return (Value){VALUE_NULL, {0}};
  ObjArray *array = (ObjArray *)AS_OBJ(args[0]);
  int idx = (int)AS_NUMBER(args[1]);
  Value val = args[2];

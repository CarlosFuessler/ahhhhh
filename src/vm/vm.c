...
static Value pop(VM *vm) { return *--vm->stack.stack_top; }

// wirft einen blick auf den obersten wert
static Value peek(VM *vm) { return vm->stack.stack_top[-1]; }
// blick uaf bestimmten wert
static Value peek_n(VM *vm, int n) { return vm->stack.stack_top[-1 - n]; }
...
#define READ_SHORT()                                                           \
  ...
  for (;;) {
    ...
    case OP_NULL:
      ...
      break;
    case OP_TRUE:
      push(vm, (Value){VALUE_BOOL, {.boolean = 1}});
      break;
    case OP_FALSE:
      push(vm, (Value){VALUE_BOOL, {.boolean = 0}});
      break;
    ...
    case OP_ADD: {
      ...
      if (IS_STRING(a) && IS_STRING(b)) {
        ...
        push(vm,
             (Value){VALUE_NUMBER, {.number = AS_NUMBER(a) + AS_NUMBER(b)}});
      } else {
    ...
    case OP_MOD: {
      ...
      push(vm,
           (Value){VALUE_NUMBER, {.number = fmod(AS_NUMBER(a), AS_NUMBER(b))}});
      break;
    }
    ...
    case OP_NOT:
      push(vm, (Value){VALUE_BOOL, {.boolean = !is_true(pop(vm))}});
      break;
    case OP_NEG:
      push(vm, (Value){VALUE_NUMBER, {.number = -AS_NUMBER(pop(vm))}});
      break;
    ...
    case OP_GET_FIELD: {
      ...
      if (IS_ARRAY(object_val) && strcmp(name->chars, "len") == 0) {
        push(vm,
             (Value){
                 VALUE_NUMBER,
                 {.number = (double)((ObjArray *)AS_OBJ(object_val))->count}});
        break;
      }

      if (IS_STRING(object_val) && strcmp(name->chars, "len") == 0) {
        push(vm, (Value){VALUE_NUMBER,
                         {.number = (double)strlen(AS_CSTRING(object_val))}});
        break;
      }
      ...
      if (!IS_OBJECT(object_val)) {
        fprintf(
            stderr,
            "Runtime error: only objects have fields. Got kind %d, field %s\n",
    ...
    case OP_STRUCT: {
      ...
      ObjObject *obj =
          (ObjObject *)allocate_obj(vm, sizeof(ObjObject), OBJ_OBJECT);
    ...
    case OP_ARRAY: {
      ...
      ObjArray *array =
          (ObjArray *)allocate_obj(vm, sizeof(ObjArray), OBJ_ARRAY);
      array->count = count;
    ...
    case OP_INDEX_GET: {
      ...
      if (IS_STRING(base_val)) {
        ...
        if (idx < 0 || idx >= len) {
          ...
          char c[2] = {s[idx], '\0'};
          push(vm,
               (Value){VALUE_STRING, {.obj = (Obj *)copy_string(vm, c, 1)}});
        }
        ...
        fprintf(stderr,
                "Runtime error: only strings and arrays can be indexed.\n");
        return INTERPRET_RUNTIME_ERROR;
      }
      break;
    }
...
static void intern_constants(VM *vm, Chunk *chunk) {
  ...
  for (int i = 0; i < chunk->constants.count; i++) {
    ...
    if (value->kind == VALUE_STRING) {
      ...
      // Die Funktion verankern, um zu verhindern, dass die GC sie bei der
      // Rekursion bereinigt
